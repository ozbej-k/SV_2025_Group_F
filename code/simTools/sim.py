...
from matrix_compare_functions import compare
...
config.PDF_ALPHA_0 = 9
config.PDF_ALPHA_W = 2
config.PDF_BETA_0 = 0.1
config.PDF_BETA_W = 1

while(config.PDF_BETA_0 <= 1):
    config.PDF_BETA_W = 0.01
    while(config.PDF_BETA_0 > config.PDF_BETA_W and config.PDF_BETA_W <= 10 ):
        ...
        #distance = compare(sim_path='simulations/Homogeneous_10AB_fast.csv', exp_path='experimental_data/Zebrafish_Positions_data/Homogeneous_10AB/*', name=f'_homo10_alpha0_{config.PDF_ALPHA_0}_alphaW_{config.PDF_ALPHA_W}_beta0_{config.PDF_BETA_0}_betaaW_{config.PDF_BETA_W}')
        #distance = compare(sim_path='simulations/Heterogeneous_10AB_fast.csv', exp_path='experimental_data/Zebrafish_Positions_data/Heterogeneous_10AB/*', name=f'_hetero10_alpha0_{config.PDF_ALPHA_0}_alphaW_{config.PDF_ALPHA_W}_beta0_{config.PDF_BETA_0}_betaaW_{config.PDF_BETA_W}')
        distance = compare(sim_path='simulations/Heterogeneous_1AB_fast.csv', exp_path='experimental_data/Zebrafish_Positions_data/Heterogeneous_1AB/*', name=f'_hetero1_alpha0_{config.PDF_ALPHA_0}_alphaW_{config.PDF_ALPHA_W}_beta0_{config.PDF_BETA_0}_betaaW_{config.PDF_BETA_W}')
        #print(f'Distance_alpha_{config.PDF_ALPHA_0}: ', distance)
        with open("simulations/histograms/distances.csv", "a") as f:
            f.write(f"{config.PDF_ALPHA_0},{config.PDF_ALPHA_W},{config.PDF_BETA_0},{config.PDF_BETA_W},{distance[0]},{distance[1]},{distance[2]},{distance[3]}\n")
        config.PDF_BETA_W += 0.01
    config.PDF_BETA_0 += 0.1
exit()
"""
def smart_search_optimization(params):
    alpha_0, alpha_w, beta_0, beta_w = params

    # Optional: limit precision
    alpha_0 = round(alpha_0, 1)
    alpha_w = round(alpha_w, 1)
    beta_0  = round(beta_0, 2)
    beta_w  = round(beta_w, 2)

    # Set config values
    config.PDF_ALPHA_0 = alpha_0
    config.PDF_ALPHA_W = alpha_w
    config.PDF_BETA_0  = beta_0
    config.PDF_BETA_W  = beta_w

    # Run simulation
    run_and_save_sim(
        tank, fishies, spots,
        60*60*10,
        "simulations/Heterogeneous_10AB_fast"
    )

    # Compare to experimental data
    distance = compare(
        sim_path="simulations/Heterogeneous_10AB_fast.csv",
        exp_path="experimental_data/Zebrafish_Positions_data/Heterogeneous_10AB/*",
        name=f"_hetero10_a0_{alpha_0}_aw_{alpha_w}_b0_{beta_0}_bw_{beta_w}"
    )

    ssim_score = distance[3]  # adjust index if needed
    with open("simulations/histograms/distances.csv", "a") as f:
        f.write(f"{config.PDF_ALPHA_0},{config.PDF_ALPHA_W},{config.PDF_BETA_0},{config.PDF_BETA_W},{distance[0]},{distance[1]},{distance[2]},{distance[3]}\n")

    print(
        f"α0={alpha_0:.2f}, αW={alpha_w:.2f}, "
        f"β0={beta_0:.2f}, βW={beta_w:.2f}, "
...
initial_guess = [16.9,20,0.15,0.01]

result = minimize(
    smart_search_optimization,
    initial_guess,
    method="Powell",
    bounds=[
        (0, 20),  # alpha_0
    ...
    options={
        "maxiter": 80,   # increase since problem is larger
        "disp": True
    }
)

best_alpha_0, best_alpha_w, best_beta_0, best_beta_w = result.x

print("\nBest parameters found:")
print(f"alpha_0 = {best_alpha_0:.2f}")
print(f"alpha_w = {best_alpha_w:.2f}")
print(f"beta_0  = {best_beta_0:.2f}")
print(f"beta_w  = {best_beta_w:.2f}")
"""
