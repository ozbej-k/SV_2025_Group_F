...
class Tank:
    ...
    def world_to_grid(self, x, y):
        ...
        if self.wall_grid is None:
            return None, None
        gx = int((x - self.xmin) / (self.xmax - self.xmin) * self.grid_width)
        gy = int((y - self.ymin) / (self.ymax - self.ymin) * self.grid_height)
        return np.clip(gx, 0, self.grid_width-1), np.clip(gy, 0, self.grid_height-1)
    ...
    def _nearest_drawn_wall(self, pos):
        ...
        gx_center, gy_center = self.world_to_grid(pos[0], pos[1])
    ...
    def _estimate_wall_tangent(self, wall_pos):
        gx, gy = self.world_to_grid(wall_pos[0], wall_pos[1])
